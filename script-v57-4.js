...
const DB_INVOICE_IMPORTS = "mypham_invoice_imports";
const DB_INVOICE_GROUPS = "mypham_invoice_groups";
const DB_INVOICE_LINES = "mypham_invoice_lines";
...
/* ======================== SUPABASE CLOUD ======================== */
const APP_VERSION = "V57.4";
const APP_BUILD_DATE = "2026-08-22";
const APP_CACHE_VERSION = "574";
const PUBLIC_NO_LOGIN_MODE = true;
...
const SUPABASE_URL = "https://qjcjljbkkmzzsqjnmyzn.supabase.co";
const SUPABASE_PUBLISHABLE_KEY = "sb_publishable_YdPKZ38JjLr237rGk1MxrA_bT2AMG7_";

const DB_ROWS = "mypham_shopee_rows";
const DB_IMPORTS = "mypham_imports";
const DB_TARGETS = "mypham_conversion_targets";
const DB_RULES = "mypham_conversion_rules";
...
async function dbReplaceShiftData(reportDate, slot, rows, importInfo) {
    ...
    const { error } = await client.rpc("mypham_replace_shift_data", {
...
function hasPermissionV40(permission) {
    // V57.3: chế độ dùng chung không cần đăng nhập.
    // Giao diện được xem như ADMIN; bảo vệ dữ liệu phụ thuộc SQL public-mode.
    if (PUBLIC_NO_LOGIN_MODE) return true;
...
async function loadUserAccessV40() {
    if (PUBLIC_NO_LOGIN_MODE) {
        state.userRole = "ADMIN";
        state.roleSource = "public-no-login";
...
function updateUserUi() {
    const email = PUBLIC_NO_LOGIN_MODE ? "Không cần đăng nhập" : (state.user?.email || "-");
...
async function enterAuthenticatedApp(session) {
    // V57.3 - KHÔNG ĐĂNG NHẬP:
    // Dùng một user ảo ở phía giao diện để các module Cloud cũ tiếp tục hoạt động.
    // Supabase truy cập bằng publishable key + chính sách public-mode trong SQL V57.3.
    state.user = {
        id: null,
        email: "Dùng chung không đăng nhập"
    };
    state.userRole = "ADMIN";
    state.roleSource = "public-no-login";
    state.roleLoaded = true;
    ...
    // Chỉ tải những dữ liệu còn được dùng trong giao diện V57.x:
    // Shopee SKU + quy đổi + thống kê hóa đơn.
    await Promise.all([
    ...
    const allowedViews = ["sku-stats", "invoice-stats"];
    ...
    // V57.3: health tối giản cho chế độ không-login, tránh gọi RPC auth cũ.
    ...
    systemHealthStateV38.checks = [
        v38HealthCheckItem(
            ...
            "ok",
            "Chế độ dùng chung không cần đăng nhập."
        ),
        v38HealthCheckItem(
            "cloud",
            "Supabase Cloud",
            state.cloudReady ? "ok" : "error",
            state.cloudReady ? "Đã đọc dữ liệu Cloud." : "Không đọc được dữ liệu Cloud. Hãy chạy SQL V57.3."
        )
    ];
...
async function saveInvoiceHistoryToCloudV39(item) {
    ...
    if (!state.user && !PUBLIC_NO_LOGIN_MODE) {
    ...
    const { data, error } = await client.rpc("mypham_save_invoice_history", {
...
async function runSystemHealthCheckV38({ silent = false } = {}) {
    ...
    systemHealthStateV38.checks = [
        v38HealthCheckItem(
            ...
            `Phiên bản ${APP_VERSION}`,
            (
                document.querySelector('link[href*="style-v57-4.css"]')?.getAttribute("href")?.includes(`v=${APP_CACHE_VERSION}`) &&
                document.querySelector('script[src*="script-v57-4.js"]')?.getAttribute("src")?.includes(`v=${APP_CACHE_VERSION}`)
            )
                ? "ok"
                : "warning",
            (
                ...
                ? `index.html đang gọi đúng style-v57-4.css?v=${APP_CACHE_VERSION} và script-v57-4.js?v=${APP_CACHE_VERSION}.`
                : `Chưa tải đúng bộ file V57.4. Hãy kiểm tra index.html, style-v57-4.css và script-v57-4.js.`
        ),
...
async function initApp() {
    try {
        initSupabaseClient();
        await enterAuthenticatedApp(null);
    } catch (error) {
        console.error("Lỗi khởi động ứng dụng public-mode:", error);

        state.user = { id: null, email: "Dùng chung không đăng nhập" };
        state.userRole = "ADMIN";
        state.roleSource = "public-no-login";
        state.roleLoaded = true;
        ...
        systemHealthStateV38.checks = [
            v38HealthCheckItem(
                ...
                "Supabase Cloud",
                "error",
                error?.message || "Không đọc được dữ liệu. Hãy chạy SQL V57.4 DÙNG CHUNG SUPABASE."
            )
        ];
        renderSystemHealthPanelV38();
        alert("Không đọc được Supabase Cloud. Hãy chạy file 1_CHAY_SQL_MY_PHAM.sql một lần trong Supabase SQL Editor.");
    }
}
