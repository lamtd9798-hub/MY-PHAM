...
async function runSystemHealthCheckV38({ silent = false } = {}) {
    ...
    systemHealthStateV38.checks = [
        v38HealthCheckItem(
            ...
            (
                document.querySelector('link[href*="style-v56-fixed.css"]')?.getAttribute("href")?.includes(`v=${APP_CACHE_VERSION}`) &&
                document.querySelector('script[src*="script-v56-fixed.js"]')?.getAttribute("src")?.includes(`v=${APP_CACHE_VERSION}`)
            )
                ? "ok"
                : "warning",
            (
                ...
                ? `index.html đang gọi đúng style-v56-fixed.css?v=${APP_CACHE_VERSION} và script-v56-fixed.js?v=${APP_CACHE_VERSION}.`
                : `Chưa tải đúng bộ file V56.1. Hãy kiểm tra index.html, style-v56-fixed.css và script-v56-fixed.js.`
        ),
