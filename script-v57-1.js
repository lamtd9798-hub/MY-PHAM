...
/* ======================== SUPABASE CLOUD ======================== */
const APP_VERSION = "V57.1";
const APP_BUILD_DATE = "2026-08-17";
const APP_CACHE_VERSION = "571";
...
/* ======================== V57.1 - TỰ ĐỘNG TRỪ ĐƠN HỦY TỪ FILE CHIỀU ======================== */
...
function getCancellationDeductionInfoV571(reportDate, morningRowsOverride = null) {
    ...
    const deductionRows = morningRows.filter(row => {
        const key = normalizeOrderIdV571(row.orderId);
        return key && cancelledOrders.has(key);
    });
...
function renderCancellationCardV571(reportDate, morningRowsOverride = null) {
    ...
    if (summary) {
        if (!getLatestImportForSlot(reportDate, "afternoon")) {
            ...
            summary.textContent = "File chiều chưa phát hiện đơn Đã hủy.";
        } else {
            summary.textContent = `${formatNumber(info.detectedOrderCount)} đơn hủy trong file chiều · ${formatNumber(info.matchedOrderCount)} đơn khớp Buổi sáng`;
        }
    }
    ...
    if (deduct) {
        deduct.textContent = info.matchedOrderCount
            ...
            : "Chỉ trừ khi Mã đơn hủy cũng tồn tại trong phần Buổi sáng đang được tính.";
    }
...
function renderCancellationDetailV571(reportDate, providedInfo = null) {
    ...
    if (summary) {
        summary.textContent = `${formatNumber(info.detectedOrderCount)} đơn hủy phát hiện trong file chiều · ${formatNumber(info.matchedOrderCount)} đơn có trong Buổi sáng và được trừ.`;
    }

    if (!info.deductionRows.length) {
        body.innerHTML = '<tr><td colspan="7" class="empty-table">Có thể có đơn hủy trong file chiều, nhưng chưa có đơn nào trùng với Buổi sáng đang được tính.</td></tr>';
        return;
    }
...
function rebuildSkuStatistics() {
    ...
    // Quan trọng: trạng thái hủy lấy từ TOÀN BỘ file chiều, nhưng chỉ trừ
    // các Mã đơn thực sự đã được cộng trong Buổi sáng hiện tại.
...
async function runSystemHealthCheckV38({ silent = false } = {}) {
    ...
    systemHealthStateV38.checks = [
        v38HealthCheckItem(
            ...
            (
                document.querySelector('link[href*="style-v57-1.css"]')?.getAttribute("href")?.includes(`v=${APP_CACHE_VERSION}`) &&
                document.querySelector('script[src*="script-v57-1.js"]')?.getAttribute("src")?.includes(`v=${APP_CACHE_VERSION}`)
            )
                ? "ok"
                : "warning",
            (
                ...
                ? `index.html đang gọi đúng style-v57-1.css?v=${APP_CACHE_VERSION} và script-v57-1.js?v=${APP_CACHE_VERSION}.`
                : `Chưa tải đúng bộ file V57.1. Hãy kiểm tra index.html, style-v57-1.css và script-v57-1.js.`
        ),
